...
namespace SGC.Clinica.Api.Application.Schedules.Handlers
{
    public class RescheduleAppointmentCommandHandler : IRequestHandler<RescheduleAppointmentCommand, AppointmentDto>
    {
        ...
        public RescheduleAppointmentCommandHandler(IApplicationDbContext context, IBaseRepository<AppointmentModel> appointmentRepository, IMapper mapper, IMediator mediator)
        {
            _context = context;
        ...
        public async Task<AppointmentDto> Handle(RescheduleAppointmentCommand request, CancellationToken cancellationToken)
        {
            ...
            appointment!.Reschedule(request.NewScheduledDate);

            await _context.SaveChangesAsync(cancellationToken);
            
            var appointmentDto = _mapper.Map<AppointmentDto>(appointment);

            await _mediator.Publish(new AppointmentRescheduledEvent(appointmentDto), cancellationToken);
